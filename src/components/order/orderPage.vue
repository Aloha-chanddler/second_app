    ...
    <el-card>
      ...
      <el-table :data="orderlist" border stripe>
        ...
        <el-table-column label="下单时间" prop="create_time">
          <!-- 使用模板插槽获取当前的下单时间 再使用之前定义好的全局时间过滤器 -->
          <template slot-scope="scope">
  ...
  export default {
    data(){
      return {
        ...
        addressFormRules:{
          address1:[
            { type:'array', required:true,message:'请选择省市区县',trigger:'blur'},
          ],
          address2:[
    ...
    methods:{
      ...
      async showProgresssBox(){
        // 发起查看物流信息的请求
        // 由于不明原因，此处请求物流进度信息会导致报错，所以进行注释，之后的代码会按照课程陆续写下去
        // const {data} = await this.$http.get(`/kuaidi/1106975712662`)
        // if(data.meta.status !== 200) return this.$message.error('请求物流信息失败！')
        // this.proressInfo = data.data
